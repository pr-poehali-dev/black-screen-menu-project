...
const AuthScreen = ({ onAuth }: AuthScreenProps) => {
  ...
  const noWebApp = !window.Telegram?.WebApp?.initData && !new URLSearchParams(window.location.search).get("token");
    ...
    <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      <div className="flex flex-col items-center gap-4 px-6 text-center">
        <img
          ...
          alt="Jaguar Casino"
          className={`w-20 h-20 object-contain ${!noWebApp && !webAppError ? "animate-pulse" : ""}`}
        />
